...
# ---------------------------------------------------------------------
# v1.8: A2A, remediation, policy, monitoring, framework, assessment pack
# ---------------------------------------------------------------------

def run_a2a_assessment_ui(a2a_json: str) -> Tuple[str, str, str, str]:
    from real_a2a_assessment.assess_a2a import assess_a2a
    payload = a2a_json.strip() if a2a_json and a2a_json.strip() else str(ROOT / "examples/a2a_trust_graph_sample.json")
    report = assess_a2a(payload, "outputs/a2a_assessments/a2a_assessment")
    md_path = ROOT / report.get("report_paths", {}).get("markdown", "outputs/a2a_assessments/a2a_assessment.md")
    js_path = ROOT / report.get("report_paths", {}).get("json", "outputs/a2a_assessments/a2a_assessment.json")
    md = md_path.read_text(encoding="utf-8") if md_path.exists() else "A2A report not generated."
    js = js_path.read_text(encoding="utf-8") if js_path.exists() else json.dumps(report, indent=2, ensure_ascii=False)
    return md, js, str(md_path), str(js_path)


def run_remediation_policy_ui(input_report_path: str) -> Tuple[str, str, str, str, str]:
    from agents.remediation_planner import build_remediation
    from agents.policy_generator import generate_policy_bundle
    path = Path(input_report_path.strip()) if input_report_path and input_report_path.strip() else ROOT / "outputs/a2a_assessments/a2a_assessment.json"
    if not path.is_absolute():
        path = ROOT / path
    report = read_json(path, {}) or {}
    remediation = build_remediation(report, "outputs/remediation/remediation_plan")
    policy = generate_policy_bundle(remediation, "outputs/policy_bundle")
    md_path = ROOT / remediation["report_paths"]["markdown"]
    js_path = ROOT / remediation["report_paths"]["json"]
    md = md_path.read_text(encoding="utf-8") if md_path.exists() else "Remediation report not generated."
    combined = {"remediation": remediation, "policy_bundle": policy.get("manifest", {})}
    return md, json.dumps(combined, indent=2, ensure_ascii=False), str(md_path), str(js_path), str(ROOT / "outputs/policy_bundle/manifest.json")


def run_runtime_monitor_ui(audit_log_path: str) -> Tuple[str, str, str, str]:
    from runtime_monitor.trace_analyzer import analyze_trace
    path = audit_log_path.strip() if audit_log_path and audit_log_path.strip() else "examples/runtime_audit_sample.jsonl"
    report = analyze_trace(str(ROOT / path) if not Path(path).is_absolute() else path, "outputs/runtime_monitor/runtime_risk_timeline")
    md_path = ROOT / report.get("report_paths", {}).get("markdown", "outputs/runtime_monitor/runtime_risk_timeline.md")
    js_path = ROOT / report.get("report_paths", {}).get("json", "outputs/runtime_monitor/runtime_risk_timeline.json")
    md = md_path.read_text(encoding="utf-8") if md_path.exists() else "Runtime monitor report not generated."
    js = js_path.read_text(encoding="utf-8") if js_path.exists() else json.dumps(report, indent=2)
    return md, js, str(md_path), str(js_path)


def run_langgraph_adapter_ui(graph_json: str) -> Tuple[str, str, str]:
    from framework_adapters.langgraph_adapter import assess_langgraph_config, load_graph
    payload = graph_json.strip() if graph_json and graph_json.strip() else str(ROOT / "examples/langgraph_agent_config_sample.json")
    report = assess_langgraph_config(load_graph(payload))
    out = ROOT / "outputs/framework_adapters/langgraph_assessment.json"
    out.parent.mkdir(parents=True, exist_ok=True)
    out.write_text(json.dumps(report, indent=2, ensure_ascii=False), encoding="utf-8")
    md = ["# LangGraph Framework Risk Extraction", "", f"- Nodes: `{report.get('nodes')}`", f"- Tools: `{report.get('tools')}`", f"- Signals: {', '.join(report.get('signals', []))}", f"- Mapped ASI: {', '.join(report.get('mapped_asi', []))}", "", "## Recommended controls"]
    md += [f"- {c}" for c in report.get("recommended_controls", [])]
    md += ["", "## Recommended benchmarks"] + [f"- `{b}`" for b in report.get("recommended_benchmarks", [])]
    return "\n".join(md), json.dumps(report, indent=2, ensure_ascii=False), str(out)


def build_assessment_pack_ui() -> Tuple[str, str, str]:
    from agents.assessment_pack_builder import build_assessment_pack
    result = build_assessment_pack("outputs/assessment_pack")
    manifest_path = ROOT / "outputs/assessment_pack/manifest.json"
    md = "# Agentic Security Assessment Pack\n\nGenerated pack includes executive summary, MCP assessment, A2A assessment, runtime monitoring, remediation, policy-as-code, and evidence appendix.\n\n"
...
""")
    ...
    with gr.Tab("13. A2A Risk Assessment"):
        ...
        a2a_json = gr.Code(language="json", label="A2A agent cards / trust graph JSON. Leave empty to use sample.", value="")
        a2a_btn = gr.Button("Run A2A risk assessment")
        a2a_md = gr.Markdown()
        a2a_report_json = gr.Code(language="json", label="A2A report JSON")
        a2a_md_file = gr.File(label="Download A2A Markdown")
        a2a_json_file = gr.File(label="Download A2A JSON")
        a2a_btn.click(run_a2a_assessment_ui, inputs=[a2a_json], outputs=[a2a_md, a2a_report_json, a2a_md_file, a2a_json_file])

    with gr.Tab("14. Remediation + Policy-as-Code"):
        gr.Markdown("Generate remediation plans and policy-as-code bundles from an assessment report.")
        rem_input = gr.Textbox(label="Assessment report JSON path", value="outputs/a2a_assessments/a2a_assessment.json")
        rem_btn = gr.Button("Generate remediation and policy bundle")
        rem_md = gr.Markdown()
        rem_json = gr.Code(language="json", label="Remediation + policy JSON")
        rem_md_file = gr.File(label="Download remediation Markdown")
        rem_json_file = gr.File(label="Download remediation JSON")
        policy_manifest_file = gr.File(label="Download policy manifest")
        rem_btn.click(run_remediation_policy_ui, inputs=[rem_input], outputs=[rem_md, rem_json, rem_md_file, rem_json_file, policy_manifest_file])

    with gr.Tab("15. Runtime Monitor"):
        gr.Markdown("Analyze audit logs for tool misuse, cascading loops, sensitive argument leakage, and egress signals.")
        audit_path = gr.Textbox(label="Audit log JSONL path", value="examples/runtime_audit_sample.jsonl")
        monitor_btn = gr.Button("Analyze runtime audit log")
        monitor_md = gr.Markdown()
        monitor_json = gr.Code(language="json", label="Runtime risk timeline JSON")
        monitor_md_file = gr.File(label="Download runtime timeline Markdown")
        monitor_json_file = gr.File(label="Download runtime timeline JSON")
        monitor_btn.click(run_runtime_monitor_ui, inputs=[audit_path], outputs=[monitor_md, monitor_json, monitor_md_file, monitor_json_file])

    with gr.Tab("16. Framework Adapter"):
        gr.Markdown("Extract agentic security surfaces from a LangGraph-style configuration.")
        graph_json = gr.Code(language="json", label="LangGraph config JSON. Leave empty to use sample.", value="")
        graph_btn = gr.Button("Assess LangGraph config")
        graph_md = gr.Markdown()
        graph_report_json = gr.Code(language="json", label="Framework assessment JSON")
        graph_file = gr.File(label="Download framework assessment JSON")
        graph_btn.click(run_langgraph_adapter_ui, inputs=[graph_json], outputs=[graph_md, graph_report_json, graph_file])

    with gr.Tab("17. Assessment Pack"):
        gr.Markdown("Build a unified enterprise-ready assessment pack combining MCP, A2A, runtime monitoring, remediation, policy, and evidence appendix.")
        pack2_btn = gr.Button("Build full assessment pack")
        pack2_md = gr.Markdown()
        pack2_json = gr.Code(language="json", label="Assessment pack manifest")
        pack2_file = gr.File(label="Download assessment pack manifest")
        pack2_btn.click(build_assessment_pack_ui, outputs=[pack2_md, pack2_json, pack2_file])

    with gr.Tab("18. Grounded Q&A"):
    ...
    with gr.Tab("19. Export / Validation Board"):
